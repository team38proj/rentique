...
$query = "SELECT p.pid, p.title, p.image, p.product_type, p.price, p.description, u.username, u.email
    FROM products p
...
if (isset($_POST['add_to_basket'])) {
    ...
    $stmt = $db->prepare("
    SELECT p.pid, p.title, p.image, p.product_type, p.price, p.description, u.username, u.email
    FROM products p
    JOIN users u ON p.uid = u.uid
    WHERE p.pid = ?
");
    ...
    <?php else: ?>
        ...
        <?php foreach ($products as $p): ?>
            <div class="product-card">
                ...
                <button type="button" class="btn secondary toggle-desc" data-target="desc-<?= $p['pid'] ?>">
                PRODUCT INFO
                </button>

                <!-- Hidden description -->
                <div id="desc-<?= $p['pid'] ?>" class="product-description" style="display:none;">
                <p> Product Info: </p><p><?= htmlspecialchars($p['description']) ?></p>
                <br>
                <p> Product Listed by: </p><p><?= htmlspecialchars($p['username']) ?></p>
                <p> Contact Seller: </p><a href="mailto:<?= htmlspecialchars($p['email']) ?>">
